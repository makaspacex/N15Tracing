    ...
    def run(self):
        while not self.k_queue.empty():
            ...
            try:
                ...
                s_time = time.time()
                str_f_s_time = core_lib.get_format_time()

                dataset = self.dataset
                mcmc_model = core_lib.get_model(dataset, self.t_eval, k_kinetics, k_sigma_priors=0.01, kf_type=0, distance=core_lib.distance_func, epsilon=core_lib.MY_EPSILON_TEE, c0_type=0)
                ...
                str_f_e_time = core_lib.get_format_time()
